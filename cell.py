...
class Cell:
    ...
    def __init__(self, x, y, thickness):
        self.x, self.y = x, y #x and y are cells coordinates
        self.thickness = thickness #thickness of the wall per cell within the maze
        self.walls = {'top': True, 'right': True, 'bottom': True, 'left': True}
        self.visited = False #visited cells are set as false

    # draw grid cell walls
    def draw(self, sc, tile): #tile is size of grid,sc is the screen
        x, y = self.x * tile, self.y * tile
        if self.walls['top']: #it draws a line on top with dg color tile with self.thickness
            pygame.draw.line(sc, pygame.Color('darkgreen'), (x, y), (x + tile, y), self.thickness)
        if self.walls['right']: #same as above
    ...
    def check_cell(self, x, y, cols, rows, grid_cells):
        find_index = lambda x, y: x + y * cols #lambda function is an anonymous function...does the operation
        if x < 0 or x > cols - 1 or y < 0 or y > rows - 1:
            return False
        return grid_cells[find_index(x, y)]#lambda funciton findindex convert 2d cods to 1d index in the grid cells list
