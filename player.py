...
class Player:
    def __init__(self, x, y):
        ...
        self.color = (250, 120, 60)#player cell is coloured as orange(rgb)
        self.velX = 0
        self.velY = 0
        self.left_pressed = False#directions of player's movements
    ...
    def check_move(self, tile, grid_cells, thickness):#tile is the cell size
        current_cell_x, current_cell_y = self.x // tile, self.y // tile#calc grid coods of player's current cell(pixels to grid cells)
        current_cell = self.get_current_cell(current_cell_x, current_cell_y, grid_cells)
        current_cell_abs_x, current_cell_abs_y = current_cell_x * tile, current_cell_y * tile#find absolute position of the current cell
        if self.left_pressed:#check for the wall on left and if movement is possible
    ...
    def update(self):
        ...
        if self.down_pressed and not self.up_pressed:
            self.velY = self.speed
        self.x += self.velX#updates the cods of player's x and y based on velecity
        self.y += self.velY
